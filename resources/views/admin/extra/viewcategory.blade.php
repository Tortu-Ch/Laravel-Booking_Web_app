...
@section('meta')
@parent
      <title>View Category Listings : Music Made Easy </title>
@endsection
@section('content')
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Category Listings</h3>
              <div style="float:right">
                <a class="btn btn-default" href="{{ URL::to('/admin/category/create') }}">Add new Category</a>
              </div>
              @if(count($category) > 0)
                ...
                @if(count($category) > 0)
                    @foreach($category as $art)
                      <tr class="pointer" id="{{ $art->id }}">
                        <td class="a-center ">
                          <input type="checkbox" name="category_check[]" value="{{ $art->id }}" class="flat categoryblk" dataid="{{ $art->id }}">
                        </td>
                        <td>{{ $art->name }}</td>
                        ...
                        <td> 
                            <a class="btn btn-info btn-flat" tooltip="" title="" type="button" href="{{ URL::to('/').'/admin/category/'.$art->id.'/edit' }}">
                              <i class="fa fa-pencil"></i>
                            </a>
                            <a class="btn btn-danger btn-flat" tooltip="" title="Delete" type="button" onclick="deletecategory({{ $art->id }});">
