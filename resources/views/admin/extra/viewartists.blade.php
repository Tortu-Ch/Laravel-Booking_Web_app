...
@section('meta')
    @parent
    <title>View Artists Listings : Music Made Easy </title>
    @endsection
    @section('content')
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">View artists</li>
        </ol>
    </section>
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">Artists Listings</h3>
                        <div style="float:right">
                            <a class="btn btn-info" href="{{ URL::to('/admin/artists/create') }}">Add new artists</a>
                        </div>
                         @if(count($artists) > 0)
                            ...
                            @if(count($artists) > 0)
                                @foreach($artists as $art)
                                    <tr class="pointer" id="{{ $art->id }}">
                                        <td class="a-center ">
                                            <input type="checkbox" name="artists_check[]" value="{{ $art->id }}" class="flat artistsblk" dataid="{{ $art->id }}">
                                        </td>
                                        <td>{{ $art->name }}</td>
                                        ...
                                        <td>
                                            <a class="btn btn-info btn-flat" tooltip="" title="" type="button" href="{{ URL::to('/').'/admin/artists/'.$art->id.'/edit' }}">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <a class="btn btn-danger btn-flat" tooltip="" title="Delete" type="button" onclick="deleteArtists({{ $art->id }});">
